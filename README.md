...
Provably-stable deep learning for nonlinear control. This repo implements composite adaptive control with a deep neural network regressor, evaluated on two classic benchmarks: a 2‑DOF two‑link robot and a 6‑state underwater vehicle (UUV) with intermittent feedback.
...
### 1) Two‑Link Robot (folder: `Two Link/`)
...
- `two_link.m`, `dynamics.m` — plant dynamics and input matrix g(x)
...
Classic adaptive controllers rely only on tracking error. Here, we augment with the DNN’s prediction error (between the learned model and inferred dynamics), yielding faster, more accurate learning while maintaining Lyapunov stability. The adaptation law uses:
...
- Optimization Toolbox (for MPC via `fmincon`)

2) Run the demos
- Two‑Link: open `Two Link/Simulate.m` in MATLAB and click Run
- UUV: open `UUV/Simulate.m` and click Run

Optional: for reproducible random initializations, add `rng(0)` near the top of `Simulate.m`.
...
## Paper
- PDF: [DNN Composite.pdf](DNN%20Composite.pdf)

If you use this work, please cite the paper/repository (update the citation if the paper title differs):
...
(Replace with the official paper citation/DOI when available.)

---

## Repo layout

- `Two Link/` — two‑link robot simulations (DNN, gradients, dynamics, controllers)
